...
(function(){
'use strict';
if(window.__SDLBuilderSimulationPathsInstalled)return;
window.__SDLBuilderSimulationPathsInstalled=true;
...
function currentPoint(packet,orig){const b=B(),resolved=resolve(packet,orig);if(!b||!resolved)return{x:orig.fx+(orig.tx-orig.fx)*(Number(packet.t)||0),y:orig.fy+(orig.ty-orig.fy)*(Number(packet.t)||0)};const a=b.nodes.find(n=>n.id===resolved.edge.from),z=b.nodes.find(n=>n.id===resolved.edge.to);if(!a||!z)return{x:orig.fx,y:orig.fy};let t=Number(packet.t)||0;if(resolved.reverse)t=1-t;const connections=window.SDLBuilderConnectionsV2;if(connections&&typeof connections.pointAt==='function'){const p=connections.pointAt(resolved.edge,t);if(p)return p;}return point(geometry(resolved.edge,a,z),t);}
function decorate(packet){if(!packet||decorated.has(packet)||packet.__sdlCurveBound)return;const orig={fx:Number(packet.fx)||0,fy:Number(packet.fy)||0,tx:Number(packet.tx)||0,ty:Number(packet.ty)||0};packet.__sdlOriginalPath=orig;packet.__sdlCurveBound=true;['fx','tx'].forEach(k=>{try{delete packet[k];Object.defineProperty(packet,k,{configurable:true,enumerable:true,get(){return currentPoint(packet,orig).x;},set(v){orig[k]=Number(v)||orig[k];}});}catch(e){}});['fy','ty'].forEach(k=>{try{delete packet[k];Object.defineProperty(packet,k,{configurable:true,enumerable:true,get(){return currentPoint(packet,orig).y;},set(v){orig[k]=Number(v)||orig[k];}});}catch(e){}});decorated.add(packet);}
function tick(){const b=B();if(b&&Array.isArray(b.simPackets))b.simPackets.forEach(decorate);requestAnimationFrame(tick);}
requestAnimationFrame(tick);
window.SDLBuilderSimulationPaths={decorate,geometry,point,currentPoint};
})();