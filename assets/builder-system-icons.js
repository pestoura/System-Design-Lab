...
function drawIconFirst(){const b=B();if(!b||!b.ctx||!spriteReady)return;const {ctx,canvas,nodes,edges,pan,zoom,selected,simPackets}=b,W=canvas.width,H=canvas.height;ctx.clearRect(0,0,W,H);ctx.fillStyle='#111827';ctx.fillRect(0,0,W,H);ctx.save();ctx.translate(pan.x,pan.y);ctx.scale(zoom,zoom);
  edges.forEach(e=>{const f=nodes.find(n=>n.id===e.from),t=nodes.find(n=>n.id===e.to);if(!f||!t)return;ensureNode(f);ensureNode(t);drawArrow(ctx,anchor(f,t),anchor(t,f),e.highlight?'#ff6f7f':'#8ab8d8',!!e.highlight);});
  nodes.filter(n=>BOUNDARY_TYPES.has(n.type)).forEach(n=>{ensureNode(n);drawBoundary(ctx,n,n===selected,vendorMeta(n));});
  nodes.filter(n=>!BOUNDARY_TYPES.has(n.type)).forEach(n=>{ensureNode(n);drawIconNode(ctx,n,n===selected,vendorMeta(n));});
